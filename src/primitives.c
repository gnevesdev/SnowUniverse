...
// I stole this function from: https://gist.github.com/derofim/912cfc9161269336f722 :P
void fill_circle(SDL_Renderer* p_renderer, int x_pos, int y_pos, int radius, uint8_t r, uint8_t g, uint8_t b, uint8_t alpha)
{
	for (int dy = 1; dy <= radius; ++dy)
	{
		...
		SDL_RenderDrawLine(p_renderer, x_pos - dx, y_pos + dy - radius, x_pos + dx, y_pos + dy - radius);
		SDL_RenderDrawLine(p_renderer, x_pos - dx, y_pos - dy + radius, x_pos + dx, y_pos - dy + radius);
	}
}
